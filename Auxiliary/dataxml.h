...
#define CNC_DIMENSION 16    //CNC维度,单独发送
...
}DataInfoTypeDef; // 所有传感器信息汇总

typedef struct
{
    HeadinfoTypeDef headInfo;
    CNCinfoTypeDef cncInfo;
}hncOnlyTypeDef;  // 数控系统信息汇总

extern DataInfoTypeDef dataInfo;  // 全局变量，传感器信息汇总
extern hncOnlyTypeDef hncInfo;  // 全局变量，数控系统信息汇总
...
public:
    ...
    static QString xmlStrCreat(DataInfoTypeDef _dataInfo); // 打包所有传感器信息到XML包
    static QString xmlHncCreat(hncOnlyTypeDef hnc848c); // 单独打包数控系统信息到XML包
    static QString xmlStrRead(QString xmlstr); // 读取网络信息
